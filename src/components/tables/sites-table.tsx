            ...
            <TableBody>
                {sites.map(site => (
                    <TableRow key={site.id}>
                        ...
                        <TableCell>{site.startDate ? site.startDate.slice(0, 10) : ''}</TableCell>
                        <TableCell>
                            {site.employeeNames || '-'}
                        </TableCell>
                        <TableCell>{site.status === 'ACTIVE' ? '稼働中' : site.status === 'COMPLETED' ? '完了' : '中止'}</TableCell>
                        <TableCell className="flex gap-2">
                            <Link href={`/sites/${site.id}`} passHref legacyBehavior>
                                <Button size="sm" variant="outline">詳細</Button>
                            </Link>
                            <Link href={`/sites/${site.id}/edit`} passHref legacyBehavior>
